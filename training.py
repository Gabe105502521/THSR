...
tensor_in = Input((48, 140, 3)) #average
...
train_data = np.stack([np.array(Image.open("./train_img/" + str(index) + ".jpg"))/255.0 for index in range(1, 8001, 1)])
...
vali_data = np.stack([np.array(Image.open("./validation_img/" + str(index) + ".jpg"))/255.0 for index in range(8001, 10001, 1)])
valicsv = open('./validation_labal.csv', 'r', encoding = 'utf8')
...
tensorBoard = TensorBoard(log_dir = './logs2', histogram_freq = 1)
